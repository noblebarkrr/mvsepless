...
from separator.audio_separator.separator import Separator
...
def custom_vr_separate(
    ...
    separator = Separator(
        ...
        output_single_stem=(
            selected_instruments[0] if len(selected_instruments) == 1 else None
        ),
    )
    output_names = audio_separator_rename_stems(
        input_file, template, model_name, model_id
    )
    ...
    separator.load_custom_vr_model(
        ...
        params={
            ...
            "vr_model_param": give_vr_params(config_path),
            "window_size": 512,
...
def main():
    ...
    custom_parser = subparsers.add_parser(
        "custom_vr", help="Custom VR model separation"
    )
    custom_parser.add_argument(
        "--input_file", required=True, help="Input audio file path"
    )
    custom_parser.add_argument(
        "--ckpt_path", required=True, help="Path to model checkpoint (.pth file)"
    )
    custom_parser.add_argument(
        "--config_path", required=True, help="Path to model config file"
    )
    custom_parser.add_argument(
        "--bitrate", type=str, default="320k", help="Output bitrate"
    )
    ...
    custom_parser.add_argument(
        "--template",
        default="{track_name}_{stem}_{model_name}",
    ...
    custom_parser.add_argument(
        "--primary_stem", default="Vocals", help="Primary stem to separate"
    )
    custom_parser.add_argument(
        "--aggression", type=int, default=5, help="Separation aggression level"
    )
    ...
    custom_parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="List of instruments to separate",
    )
    ...
    uvr_parser.add_argument(
        "--model_dir", required=True, help="Directory containing model files"
    )
    uvr_parser.add_argument(
        "--model_type",
        required=True,
        choices=["mdx", "vr"],
        help="Model type (mdx or vr)",
    )
    ...
    uvr_parser.add_argument(
        ...
        type=int,
        default=5,
        help="Separation aggression level (for VR models)",
    )
    uvr_parser.add_argument(
        "--selected_instruments",
        nargs="*",
