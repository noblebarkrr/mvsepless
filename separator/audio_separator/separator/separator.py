...
class Separator:
    ...
    def __init__(
        ...
        if not info_only:
            self.logger.info(f"Separator version 0.32.0 instantiating with output_dir: {output_dir}, output_format: {output_format}")
    ...
    def load_custom_vr_model(self, model_path, config_path=None, params=None):
        ...
        self.primary_stem_output_path = separator.model_instance.primary_stem_output_path
        self.secondary_stem_output_path = separator.model_instance.secondary_stem_output_path
...
class CustomModelLoader:
    ...
    def load_custom_vr_model(self, model_path, config_path=None, params=None):
        ...
        """
        self.logger.info(f"Loading custom VR model from {model_path}")
        ...
        if "vr_model_param" not in model_data:
            raise ValueError("VR models require 'vr_model_param' in model data")
    ...
    def load_custom_mdx_model(self, model_path, config_path=None, params=None):
        ...
        Returns:
            dict: Model data dictionary containing all necessary parameters for separation.
        """
        self.logger.info(f"Loading custom MDX model from {model_path}")
        ...
        try:
            with open(config_path, 'r', encoding='utf-8') as f:
                if config_path.lower().endswith('.json'):
                    return json.load(f)
                elif config_path.lower().endswith('.yaml') or config_path.lower().endswith('.yml'):
    ...
    def prepare_custom_model(self, model_data):
        ...
        if model_type not in ["VR", "MDX"]:
            raise ValueError("Unsupported model type. Only VR and MDX are supported for custom loading.")
        ...
        if model_type == "VR":
            ...
            for key in required_keys:
                if key not in model_data:
                    raise ValueError(f"VR model missing required parameter: {key}")
        ...
        common_params = {
            ...
            "use_soundfile": self.separator.use_soundfile,
        }
        ...
        self.separator.model_instance = separator_class(common_config=common_params, arch_config=arch_config)
