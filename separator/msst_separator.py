...
def once_inference(
    ...
    try:
        mix, sr = audio.read(i=path, sr=sample_rate, mono=False)
    except Exception as e:
    ...
    for instr in instruments:
        try:
            ...
            custom_name = output_file_template(
                template, file_name, instr, model_name, model_id
            )
...
def run_inference(
    ...
    if config.training.target_instrument is not None:
        print(
            "Целевой инструмент найден в конфигурации модели. Выбранные стемы будут проигнорированы."
        )
    else:
...
def load_model(model_type, config_path, start_check_point, device_ids, force_cpu=False):
    ...
    print(
        f"Потрачено времени на загрузку модели: {time.time() - model_load_start_time:.2f} сек."
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--input_list", nargs="+", help="Список с путями к входным файлам"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="wav",
        choices=["wav", "mp3", "flac", "m4a", "aac", "aiff", "ogg", "opus"],
    ...
    parser.add_argument(
        "--batch", action="store_true", help="Обработать все файлы в папке"
    )
    parser.add_argument(
        "--batch_list", action="store_true", help="Обработать все файлы в списке"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save_results_info",
        action="store_true",
        help="Сохранить данные разделения в {args.store_dir}/results.json для отображения в интерфейсе",
    )
