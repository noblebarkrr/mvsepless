...
- Обновлены существующие плагины:
   ...
   - MVSEP API:
     - Добавлено форматирование имени выходного файла
     - Динамические плееры заменены на те, которые используются в основном инференсе
     - Изменены переменные окружения

# Фишки:

- NEW! Отправка результата разделения на вход (типа, как кнопка "Reuse this file" рядом с каждым файлом на MVSEP)
...
- Модель Mel-Band_Roformer WSA от Smulelabs плохо протестирована
...
- **MVSEP API** - Плагин-клиент для MVSEP<br><br>***Плагин отключается при любом заданном значении переменной окружения MVSEP_API_OFF***<br><br>***NEW! Теперь можно задать токен авторизации при запуске с помощью перменной окружения MVSEP_API_TOKEN***<br><br>*(Встроено в репозиторий, загружать его отдельно не требуется)*<br> <br>
...
MVSEP_API_OFF - деактивация плагина (любое значение)

MVSEP_API_TOKEN - токен авторизации для MVSEP (https://mvsep.com/full_api)
