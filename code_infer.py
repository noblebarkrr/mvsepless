...
def code_infer():
    ...
    if infer == "mss":
        ...
        medley_infer = ("python", "-m", "models.medley_vox.svs.inference", "--inference_data_dir", str(args.input), "--results_save_dir", str(args.output), "--model_dir", str(model_dir_medley_vox), "--exp_name", str(args.modelcode), "--use_overlapadd=ola", ('--batch' if args.batch else '') )
