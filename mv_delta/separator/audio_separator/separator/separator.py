...
class Separator:
    ...
    def __init__(
        ...
        if output_dir is None:
            ...
            if not info_only:
                self.logger.info(
                    "Выходная директория не указана. Используется текущая рабочая директория"
                )
        ...
        if env_model_dir:
            ...
            self.logger.info(
                f"Используется директория кэша моделей из переменной окружения AUDIO_SEPARATOR_MODEL_DIR: {self.model_file_dir}"
            )
            if not os.path.exists(self.model_file_dir):
                raise FileNotFoundError(
                    f"Указанной директории кэша моделей не существует: {self.model_file_dir}"
                )
        else:
            self.logger.info(
                f"Используется директория кэша моделей из параметра model_file_dir: {model_file_dir}"
            )
        ...
        if normalization_threshold <= 0 or normalization_threshold > 1:
            raise ValueError(
                "normalization_threshold должен быть в пределах от 0 до 1"
            )

        self.amplification_threshold = amplification_threshold
        if amplification_threshold < 0 or amplification_threshold > 1:
            raise ValueError(
                "amplification_threshold должен быть в пределах от 0 до 1"
            )
        ...
        if self.invert_using_spec:
            self.logger.debug(
                f"Второстепенный стем получен с помощью вычитания первого стема из спектрограммы оригинала"
            )
        ...
        try:
            ...
            if self.sample_rate <= 0:
                raise ValueError(
                    f"Частота дискретизации не должна быть нулевой или отрицательной"
                )
            if self.sample_rate > 192000:
                raise ValueError(
                    f"Слишком высокая частота дискретизации"
                )
        except ValueError:
            raise ValueError(
                "Недействительная частота дискретизации. Введите положительное число"
            )
        ...
        self.custom_loader = CustomModelLoader(self)
    ...
    def get_system_info(self):
        ...
        self.logger.info(f"Операционная система: {os_name} {os_version}")

        system_info = platform.uname()
        self.logger.info(
            f"Система: {system_info.system} Нода: {system_info.node} Релиз: {system_info.release} Машина: {system_info.machine} ЦП: {system_info.processor}"
        )

        python_version = platform.python_version()
        self.logger.info(f"Версия Python: {python_version}")

        pytorch_version = torch.__version__
        self.logger.info(f"Версия PyTorch: {pytorch_version}")
        return system_info
    ...
    def log_onnxruntime_packages(self):
        ...
        if onnxruntime_gpu_package is not None:
            self.logger.info(
                f"Версия ONNX Runtime ГПУ: {onnxruntime_gpu_package.version}"
            )
        if onnxruntime_silicon_package is not None:
            self.logger.info(
                f"Версия ONNX Runtime Silicon: {onnxruntime_silicon_package.version}"
            )
        if onnxruntime_cpu_package is not None:
            self.logger.info(
                f"Версия ONNX Runtime {onnxruntime_cpu_package.version}"
            )
    ...
    def setup_torch_device(self, system_info):
        ...
        if not hardware_acceleration_enabled:
            self.logger.info(
                "Аппаратное ускорение не настроено. Используется ЦП"
            )
    ...
    def configure_cuda(self, ort_providers):
        ...
        """
        self.logger.info("CUDA доступно в torch")
        self.torch_device = torch.device("cuda")
        if "CUDAExecutionProvider" in ort_providers:
            self.logger.info(
                "в ONNXruntime доступен CUDAExecutionProvider, включается ускорение"
            )
            self.onnx_execution_provider = ["CUDAExecutionProvider"]
        else:
            self.logger.warning(
                "CUDAExecutionProvider не доступен ONNXruntime, ускорение не может быть включено"
            )
    ...
    def configure_mps(self, ort_providers):
        ...
        """
        self.logger.info(
            "Apple Silicon MPS/CoreML доступен в Torch"
        )
        ...
        if "CoreMLExecutionProvider" in ort_providers:
            self.logger.info(
                "в ONNXruntime доступен CoreMLExecutionProvider, включается ускорение"
            )
            self.onnx_execution_provider = ["CoreMLExecutionProvider"]
        else:
            self.logger.warning(
                "CoreMLExecutionProvider не доступен в ONNXruntime, ускорение не может быть включено"
            )
    ...
    def get_package_distribution(self, package_name):
        ...
        try:
            ...
            self.logger.debug(f"Пакет: {package_name} не установлен")
            return None
    ...
    def get_model_hash(self, model_path):
        ...
        """
        self.logger.debug(f"Вычисляется хэш файла модели {model_path}")
        ...
        try:
            ...
            with open(model_path, "rb") as f:
                if file_size < BYTES_TO_HASH:
                    ...
                    self.logger.debug(
                        f"Размер {file_size} < {BYTES_TO_HASH}, hashing entire file."
                    )
                    ...
                    self.logger.debug(
                        f"Размер {file_size} >= {BYTES_TO_HASH}, seeking to {seek_pos} and hashing remaining bytes."
                    )
                    f.seek(seek_pos, io.SEEK_SET)
                    hash_value = hashlib.md5(f.read()).hexdigest()

            # Log the calculated hash
            self.logger.info(f"Хэш {model_path} это {hash_value}")
            return hash_value

        except FileNotFoundError:
            self.logger.error(f"Модели не найдено в этом пути: {model_path}")
            raise  # Re-raise the specific error
        except Exception as e:
            # Catch other potential errors (e.g., permissions, other IOErrors)
            self.logger.error(f"Не удалось вычислить хэш модели {model_path}: {e}")
    ...
    def download_file_if_not_exists(self, url, output_path):
        ...
        if response.status_code == 200:
            ...
            raise RuntimeError(
                f"Загрузка с {url} не удалась, код: {response.status_code}"
            )
    ...
    def download_model_files(self, model_filename):
        ...
        raise ValueError(
            f"Эта модель не найдена: {model_filename}"
        )
    ...
    def load_model_data_from_yaml(self, yaml_config_filename):
        ...
        self.logger.debug(
            f"Данные модели загружены из YAML: {model_data_yaml_filepath}"
        )

        model_data = yaml.load(
            open(model_data_yaml_filepath, encoding="utf-8"), Loader=yaml.FullLoader
        )
        self.logger.debug(f"Данные модели загружены из YAML: {model_data}")
    ...
    def load_model_data_using_hash(self, model_path):
        ...
        if model_hash in mdx_model_data_object:
            ...
            raise ValueError(
                f"Неподдерживаемое имя модели"
            )

        self.logger.debug(f"Модель загружена, используя хэш {model_hash}: {model_data}")
    ...
    def load_model(
        ...
        """
        self.logger.info(f"Загружается модель {model_filename}...")
        ...
        self.logger.debug(
            f"Модель загружена, понятное имя: {model_friendly_name}, путь: {model_path}"
        )
        ...
        if (
            ...
            raise ValueError(f"Не поддерживаемый тип модели: {model_type}")
        ...
        self.logger.info(
            f'Потрачено на загрузку модели: {time.strftime("%H:%M:%S", time.gmtime(int(time.perf_counter() - load_model_start_time)))}'
        )
    ...
    def separate(self, audio_file_path, custom_output_names=None):
        ...
        if not (self.torch_device and self.model_instance):
            raise ValueError(
                "Инициализация не удалась или модель не была загружена. Сначала загрузите модель перед разделением"
            )
        ...
        for path in audio_file_path:
            if os.path.isdir(path):
                ...
                for root, dirs, files in os.walk(path):
                    for file in files:
                        ...
                        if file.endswith(
                            ...
                            self.logger.info(f"Обработка файла: {full_path}")
                            try:
                                ...
                                self.logger.error(
                                    f"Не удалось обработать файл {full_path}: {e}"
                                )
            else:
                ...
                try:
                    ...
                    self.logger.error(f"Не удалось обработать файл {path}: {e}")
    ...
    def _separate_file(self, audio_file_path, custom_output_names=None):
        ...
        self.logger.info(
            f"Начинается разделение для audio_file_path: {audio_file_path}"
        )
        ...
        if self.use_autocast and autocast_mode.is_autocast_available(
            ...
            self.logger.debug("Autocast доступен")
            ...
            self.logger.debug("Autocast недоступен")
            output_files = self.model_instance.separate(
                audio_file_path, custom_output_names
            )
        ...
        self.logger.debug("Разделение завершено")
        self.logger.info(
            f'Потрачено на разделение: {time.strftime("%H:%M:%S", time.gmtime(int(time.perf_counter() - separate_start_time)))}'
        )
    ...
    def download_model_and_data(self, model_filename):
        ...
        """
        self.logger.info(f"Скачивается модель {model_filename}...")
        ...
        self.logger.info(
            f"Модель скачана, тип: {model_type}, понятное имя: {model_friendly_name}, путь: {model_path}, данные модели: {model_data_dict_size} предметов"
        )
    ...
    def load_custom_vr_model(self, model_path, config_path=None, params=None):
        model_data = self.custom_loader.load_custom_vr_model(
            model_path, config_path, params
        )
        separator = self.custom_loader.prepare_custom_model(model_data)

        # Устанавливаем критические атрибуты из кастомной модели
        self.primary_stem_name = separator.model_instance.primary_stem_name
        self.secondary_stem_name = separator.model_instance.secondary_stem_name
        self.primary_stem_output_path = (
            separator.model_instance.primary_stem_output_path
        )
        self.secondary_stem_output_path = (
            separator.model_instance.secondary_stem_output_path
        )

        return separator

    def load_custom_mdx_model(self, model_path, config_path=None, params=None):
        model_data = self.custom_loader.load_custom_mdx_model(
            model_path, config_path, params
        )
        separator = self.custom_loader.prepare_custom_model(model_data)
...
class CustomModelLoader:
    """
    This class handles custom model loading for VR and MDX architectures, allowing users to provide
    their own model files, checkpoints, configs, and parameters.
    """

    def __init__(self, separator_instance):
        """
        Initialize the CustomModelLoader with a reference to the Separator instance.

        Args:
            separator_instance (Separator): An instance of the Separator class to access its methods and attributes.
        """
        self.separator = separator_instance
        self.logger = separator_instance.logger

    def load_custom_vr_model(self, model_path, config_path=None, params=None):
        """
        Load a custom VR model with optional config and parameters.

        Args:
            model_path (str): Path to the VR model file (.pth or .onnx).
            config_path (str, optional): Path to the model config file (JSON or YAML).
            params (dict, optional): Dictionary of parameters to override defaults.

        Returns:
            dict: Model data dictionary containing all necessary parameters for separation.
        """
        self.logger.info(f"Загружается кастомная VR модель из {model_path}")

        # Load or create model data
        if config_path:
            model_data = self._load_config_file(config_path)
        else:
            model_data = self._create_default_vr_model_data()

        # Apply any custom parameters
        if params:
            model_data.update(params)

        # Validate required VR parameters
        if "vr_model_param" not in model_data:
            raise ValueError("VR модель требует 'vr_model_param' в model_data")

        # Set model-specific properties
        model_data["model_path"] = model_path
        model_data["model_type"] = "VR"

        return model_data

    def load_custom_mdx_model(self, model_path, config_path=None, params=None):
        """
        Load a custom MDX model with optional config and parameters.

        Args:
            model_path (str): Path to the MDX model file (.onnx).
            config_path (str, optional): Path to the model config file (JSON or YAML).
            params (dict, optional): Dictionary of parameters to override defaults.
        ...
        Returns:
            dict: Model data dictionary containing all necessary parameters for separation.
        """
        self.logger.info(f"Загружается кастомная MDX модель из {model_path}")

        # Load or create model data
        if config_path:
            model_data = self._load_config_file(config_path)
        else:
            model_data = self._create_default_mdx_model_data()
    ...
    def _load_config_file(self, config_path):
        ...
        Returns:
            dict: Loaded model configuration data.
        """
        self.logger.debug(f"Загружается конфиг модели из {config_path}")

        try:
            with open(config_path, "r", encoding="utf-8") as f:
                if config_path.lower().endswith(".json"):
                    return json.load(f)
                elif config_path.lower().endswith(
                    ".yaml"
                ) or config_path.lower().endswith(".yml"):
                    return yaml.safe_load(f)
                else:
                    raise ValueError(
                        "Неподдерживаемый формат конфига. Используйте JSON или YAML."
                    )
        except Exception as e:
            self.logger.error(f"Failed to load config file: {e}")
            raise

    def _create_default_vr_model_data(self):
        """
        Create default model data structure for VR models.

        Returns:
            dict: Default VR model data structure.
        """
        return {
            "primary_stem": "Vocals",
            "secondary_stem": "Instrumental",
            "is_vr_model": True,
            "vr_model_param": "1band_sr44100_hl512",  # Required for VR models
            "n_fft": 2048,
            "hop_length": 512,
            "window_size": 512,
            "batch_size": 1,
    ...
    def _create_default_mdx_model_data(self):
        """
        Create default model data structure for MDX models.

        Returns:
            dict: Default MDX model data structure.
        """
        return {
            "primary_stem": "Vocals",
            "secondary_stem": "Instrumental",
    ...
    def prepare_custom_model(self, model_data):
        """
        Prepare the separator instance to use a custom loaded model.

        Args:
            model_data (dict): Model data dictionary containing all necessary parameters.

        Returns:
            Separator: The configured separator instance ready for separation.
        """
        model_type = model_data.get("model_type")
        if model_type not in ["VR", "MDX"]:
            raise ValueError(
                "Не поддерживаемый тип модели для кастомной загрузки"
            )

        # For VR models, validate required parameters
        if model_type == "VR":
            required_keys = ["vr_model_param", "window_size", "aggression"]
            for key in required_keys:
                if key not in model_data:
                    raise ValueError(f"У VR модели отсутствует обязательный параметр: {key}")

        # Set common parameters
        common_params = {
            "logger": self.separator.logger,
            "log_level": self.separator.log_level,
            "torch_device": self.separator.torch_device,
            "torch_device_cpu": self.separator.torch_device_cpu,
            "torch_device_mps": self.separator.torch_device_mps,
            "onnx_execution_provider": self.separator.onnx_execution_provider,
            "model_name": os.path.basename(model_data["model_path"]).split(".")[0],
            "model_path": model_data["model_path"],
            "model_data": model_data,
            "output_format": self.separator.output_format,
            "output_bitrate": self.separator.output_bitrate,
            "output_dir": self.separator.output_dir,
            "normalization_threshold": self.separator.normalization_threshold,
            "amplification_threshold": self.separator.amplification_threshold,
            "output_single_stem": self.separator.output_single_stem,
            "invert_using_spec": self.separator.invert_using_spec,
        ...
        if model_type == "VR":
            arch_config = {
                ...
                "window_size": model_data.get("window_size", 512),
                "aggression": model_data.get("aggression", 5),
                "enable_tta": model_data.get("enable_tta", False),
                "enable_post_process": model_data.get("enable_post_process", False),
                "post_process_threshold": model_data.get("post_process_threshold", 0.2),
                "high_end_process": model_data.get("high_end_process", False),
            }
        elif model_type == "MDX":
            arch_config = {
                "hop_length": model_data.get("hop_length", 1024),
                "segment_size": model_data.get("segment_size", 256),
                "overlap": model_data.get("overlap", 0.25),
                "batch_size": model_data.get("batch_size", 1),
                "enable_denoise": model_data.get("enable_denoise", False),
            }

        # Import and instantiate the appropriate separator class
        separator_classes = {
            "MDX": "mdx_separator.MDXSeparator",
            "VR": "vr_separator.VRSeparator",
        }
        ...
        self.separator.model_instance = separator_class(
            common_config=common_params, arch_config=arch_config
        )

        # Принудительно инициализируем атрибуты путей
        self.separator.model_instance.primary_stem_output_path = None
        self.separator.model_instance.secondary_stem_output_path = None
        self.separator.model_instance.audio_file_path = None
        self.separator.model_instance.audio_file_base = None
