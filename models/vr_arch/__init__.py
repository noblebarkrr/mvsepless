...
from . import spec_utils, nets, nets_new, nets_v6
...
def get_model(config):
    ...
    if hasattr(config.model, "is_vr6"):
        if config.model.is_vr6:
            is_vr_6_model = True

    if is_vr_6_model:
        model = nets_v6.CascadedNet(n_fft=config.model.n_fft, hop_length=config.model.hop_length, nout=model_capacity[0], nout_lstm=model_capacity[1], is_complex=config.model.complex)
    else:
